...
export const accessibilityPropsChallenges: Challenge[] = [
  ...
  'aria-live'?: 'polite' | 'assertive' | 'off';
    ...
    explanationWrong:
      '`role?: string` accepts `"banana"` without complaint. ARIA roles must be from a defined set, and an invalid role is silently ignored by the browser, and the element loses its semantic meaning.\n\nUnion types apply the same constraint strategy used for `size` and `variant` props: narrow the type to only valid values.',
    sourceUrl:
